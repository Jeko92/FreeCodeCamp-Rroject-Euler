const largestProductinaSeries = (n) => {
  ...
  let groupsOfFor = [];
  for (let i = 0; i < thousandDigits.length - 3; i++) {
    let temp = [];
    temp.push(thousandDigits[i]);
    //console.log(`I: ${i}`);
    for (let j = i + 1; j < i + n; j++) {
      temp.push(thousandDigits[j]);
      //console.log(`J: ${j}`);
    }
    groupsOfFor.push(temp);
    temp = [];
  }
  //console.log(max);
  console.log(groupsOfFor);
  groupsOfFor.map((arr) => arr.reduce((a, b) => a * b, 1));
  console.log(groupsOfFor.map((arr) => arr.reduce((a, b) => a * b, 1)));
  console.log(
    Math.max(...groupsOfFor.map((arr) => arr.reduce((a, b) => a * b, 1)))
  );
  console.log(
    Number.isNaN(
      groupsOfFor
